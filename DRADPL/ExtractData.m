...
%     TrData(:,(i-1)*number+1:i*number) = Tempdatai(:,Index(1:number));
%     TtData(:,(i-1)*(L-number)+1:i*(L-number)) = Tempdatai(:,Index((number+1):end));
%     Trlabel(:,(i-1)*number+1:i*number) = Templabeli(:,Index(1:number));
%     Ttlabel(:,(i-1)*(L-number)+1:i*(L-number)) = Templabeli(:,Index((number+1):end));
%     
% end
%% split data
lab = unique(labels);
